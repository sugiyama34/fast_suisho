...
1. ✅ **AccumulatorCache (finny tables) の移植** → **完了 (2026-07-31,
   ...
   完全一致)。NPS 1T **+38.7〜41.4%** (finny 単体 +28.3〜29.8%、update 経路の
   accumulator 値渡しコピー除去で +7.4〜10.0% 上乗せ)、16T +36.3〜37.0% (暫定 —
   GPU 学習ジョブ同時実行下の計測。アイドル環境と c8a で再計測予定)。
...
3. ⬜ **fc 層のスパース化** (fc_0 は既に sparse-input 実装。fc_1 64ch の刈り込み)
   — 後段 FC はコスト比率が小さく NPS 効果は数%未満の見込み (experiment-002 実測: fc_1 は 0.6%)。
   優先度低 (サーベイ §2.2)
...
6. ⏸️ **FT の int8 化** — 重み 258→129 MiB で帯域半減。エンジン改造 (大) + 精度感度が高く
   QAT 前提。experiment-002 実測では IPC 2.24 と完全な帯域律速ではないため、finny tables (着手順 1) と

...
進捗マーク: ✅ 完了 / 🏃‍♀️ 進行中 / 🔜 次に着手 / ⬜ 未着手 / ⏸️ 条件待ち / ❌ 除外
...
着手順 (0 の残り → 1 を先行。効果が見込める順。2026-07-14 に実験 002 の実測を反映して更新):
...
0. 🏃‍♀️ **プロファイル取得と精度リスクゼロ系** (サーベイ §2.3, §2.7)
   ...
   - 🔜 FT 出力チャネル並べ替え (activation-sparsity permutation): 等価変換で精度損ゼロ、
     ...
     **連続 4ch 単位 (int32 チャンク)**、並べ替えの自由度は pairwise-mul のペア
     (j, j+512) 単位と確認済み (実験 002)。NPS 上限は +2% 程度と小さいが、
     weights-only 加工パイプラインの練習台を兼ねて実験 003 で実施
   - ⏸️ PGO / huge pages 等のビルド最適化: 採用する場合は**ベースラインにも適用して再計測**
     (ネットワーク手法の効果と混ぜない)。実験 002 実測の dTLB ミス 2.4M/s から
...
1. ⬜ **AccumulatorCache (finny tables) の移植** — 実験 002 の実測を受け 2026-07-14 に追加・昇格
...
3. ⬜ **fc 層のスパース化** (fc_0 は既に sparse-input 実装。fc_1 64ch の刈り込み)
   — 後段 FC はコスト比率が小さく NPS 効果は数%未満の見込み (実験 002 実測: fc_1 は 0.6%)。
   優先度低 (サーベイ §2.2)
...
6. ⏸️ **FT の int8 化** — 重み 258→129 MiB で帯域半減。エンジン改造 (大) + 精度感度が高く
   QAT 前提。実験 002 実測では IPC 2.24 と完全な帯域律速ではないため、finny tables (着手順 1) と
   FT 幅縮小 (着手順 2) の後に残る帯域コストを見てから検討

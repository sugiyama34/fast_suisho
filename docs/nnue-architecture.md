...
視点 $p \in \{\text{手番側}, \text{相手側}\}$ ごとに、局面を **ちょうど 40 個の 1** を持つ
二値スパースベクトル $x^p \in \{0,1\}^{131949}$ で表す
(盤上・持ち駒の全駒 1 つにつき 1 特徴。**両玉を含む** — HalfKA の "A" = All pieces):

$$\text{active}(x^p) = \{\, \text{index}(\text{sq}_{\text{自玉}},\ \text{BonaPiece}) \mid \text{全 40 駒} \,\}, \qquad 131949 = 81 \times 1629$$
...
$$z^p_j = \left\lfloor \frac{\mathrm{clamp}(u_j, 0, 127) \cdot \mathrm{clamp}(v_j, 0, 127)}{128} \right\rfloor \qquad z^p \in \{0,\dots,126\}^{512}\ (\text{uint8})$$

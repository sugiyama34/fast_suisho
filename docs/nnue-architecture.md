...
視点 $p \in \lbrace \text{手番側}, \text{相手側} \rbrace$ ごとに、局面を二値スパースベクトル
$x^p \in \lbrace 0,1 \rbrace^{131949}$ で表す。この 131,949 次元は平坦な 1 本のベクトルではなく、
**「自玉のマス (81 通り) × 駒状態 (1629 通り)」の 2 次元格子**を平坦化したもの
($131949 = 81 \times 1629$) と読む:

- 自玉の位置に応じて、81 個ある**ブロック (各 1629 次元) のうち 1 つ**が選ばれる
- 選ばれたブロックの中で、盤上・持ち駒の**全 40 駒に対応するちょうど 40 個**の要素が 1 になる
  (1 駒につき 1 特徴。**両玉を含む** — HalfKA の "A" = All pieces)
- 残りの 80 ブロック (80 × 1629 次元) は**すべて 0**

$$
\text{active}(x^p) = \lbrace \text{index}(\text{sq}_{\text{自玉}},\ \text{BonaPiece}(i)) \mid i \in \text{全 40 駒} \rbrace
$$

自玉が動くと、40 個の 1 がブロックごと別の 1629 次元スライスへ移る —
後述の accumulator 全再計算 (refresh) が必要になるのはこのためである。

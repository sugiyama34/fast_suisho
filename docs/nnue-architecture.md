...
二値スパースベクトル $x^p \in \lbrace 0,1 \rbrace^{131949}$ で表す
(盤上・持ち駒の全駒 1 つにつき 1 特徴。**両玉を含む** — HalfKA の "A" = All pieces):

$$\text{active}(x^p) = \lbrace \text{index}(\text{sq}_{\text{自玉}},\ \text{BonaPiece}) \mid \text{全 40 駒} \rbrace, \qquad 131949 = 81 \times 1629$$
...
$$\text{index}(k, p) = 1629 \cdot k + p', \qquad p' = \begin{cases} p - 81 & (p \ge 1629 \text{ 、つまり敵玉}) \\ p & (\text{それ以外}) \end{cases}$$

- $k$ = 自玉の升 (0–80)。後手視点では盤を 180° 回転した正規化座標を使う
...
- 81 升 × 1629 駒状態の one-hot 格子を平坦化したものが特徴量番号になる。
...
| 90–1547 | **盤上の駒 (玉以外)**: (駒種×所有者) ごとに 81 幅のブロック、`id = base + 升` (１一 = 0 … ９九 = 80)。順に 歩/香/桂/銀/金/角/馬/飛/竜 × f/e。**馬・竜は独立ブロック**だが、と金・成香・成桂・成銀は**金のブロックに合流** (`DISTINGUISH_GOLDS` 無効のため。これが `fe_end` = 1548 の理由) |
| 1548–1628 | **玉の平面**: `f_king + 升`。enum 上は敵玉に 1629–1709 が割り当てられているが、`MakeIndex` が 81 を引いて**この平面に折り畳む** (自玉の特徴は必ず対角成分 $p' - 1548 = k$ になるため衝突しない)。幅が 1710 でなく 1629 で済むのはこのため |

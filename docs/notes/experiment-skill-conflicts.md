...
| ファイル構成 | `hypothesis.md` / `metadata.json` / `config/` / `command.md` / `logs/` / `metrics/` / `figures/` / `report.md` に分割 | `report.md` 1 本に仮説・セットアップ・再現手順・結果・結論を集約し、スクリプトと成果物 (png / npy / txt) を同フォルダに直置き (実験 000 / 実験 001 の既存形式) |

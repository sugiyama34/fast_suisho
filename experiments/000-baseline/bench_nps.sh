...
for t in $THREADS_LIST; do
  if ! out=$(printf 'setoption name EvalDir value %s\nbench %s %s %s default movetime\nquit\n' \
               "$EVALDIR" "$HASH_MB" "$t" "$MOVETIME" | "$ENGINE" 2>&1); then
    echo "error: engine exited nonzero at threads=$t; output was:" >&2
    echo "$out" >&2
    exit 1
  fi
  ...
  ms=$(echo "$out"    | grep -oE 'Total time \(ms\) : [0-9]+' | grep -oE '[0-9]+' | tail -1 || true)

...
`./bench_nps.sh 10000` (やねうら王 bench デフォルト4局面 × movetime 10秒, hash 1024MB):
...
- **最大 NPS ≈ 7.19M (16スレッド)**。EPYC 7302 は 16 物理コア / SMT なしのため 16 が上限
- スケーリングはほぼ線形 → 探索は綺麗に並列化されており、評価関数速度が支配的
- 1スレッド値 (466k) が冒頭のアドホック値 (600k) より低いのは、bench 局面集に

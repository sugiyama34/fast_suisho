...
- 計測ワークロード: 000-baseline と同一の bench (デフォルト4局面 × movetime 10秒)、
  **1 スレッド** (アトリビューションを明瞭にするため。experiment-000 でスケーリングがほぼ線形
...
- → **並べ替えの目標は「同時にゼロになりやすいチャネルを同じ 4ch チャンクに固める」**こと。
  ...
  FT チャネルのペア (j, j+512) 単位。実装詳細は experiment-003 で確定する
...
2. **開発機 16T は「軽い競合」域**: IPC -6%、LLC ミス率 +1.5pt と競合の兆候は
   出るが比率はほぼ動かない。これは experiment-000 の線形スケーリング (15.34×/16T) と整合

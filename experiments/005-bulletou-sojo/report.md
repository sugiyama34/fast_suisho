...
## 用語: 「BulletOu-epoch」と「standard-epoch」

本レポート群では「epoch」の多義性が結論に直結するため、以下の 2 語を区別して使う
(2026-07-28 ユーザー指示で明文化):

- **BulletOu-epoch**: BulletOu が `--superbatches N` 明示時に「epoch」と呼ぶ単位。
  **N superbatch = N × 40M 局面の LR/検証サイクル**であり、データ量とは無関係。
  本実験 (N=12) では 1 BulletOu-epoch = 約 4.8 億局面 = 教師全体の約 3%。
  LR の step 減衰はこの単位で 1 周し、境界で warm restart する
- **standard-epoch**: 機械学習の通常の意味 = **教師データ全体を 1 周** (奏乗全 30
  ファイル = 約 146.7 億局面)。1 standard-epoch ≒ 30.6 BulletOu-epoch (N=12 換算)

本実験 (005) の学習量: **16 BulletOu-epoch = 76.8 億局面 ≒ 0.52 standard-epoch**。
つまり「16 epoch 学習した」は BulletOu-epoch の意味であり、standard-epoch では
半周にも満たない。この区別が「2 日くらい学習」との乖離の最有力説明である
(詳細は「解釈」「次のステップ」参照)。
...
計画 (2026-07-28 ユーザー確定版。**実行は PR #14 マージ後**):

0. **準備 (マージ前に実施)**: 教師の残り 13 ファイル (~254 GB) をダウンロードし
   全 30 ファイル = 146.7 億局面を揃える (実施済み・DL 進行中)。
   用語 BulletOu-epoch / standard-epoch を全レポートに明記 (本レポート冒頭参照)
1. **フェーズ a — 2^n BulletOu-epoch checkpoint の Elo 測定 (GPU 不要)**:
   exp-005 保存済み `005-main/0001, 0002, 0004, 0008, 0016` (= 1/2/4/8/16
   BulletOu-epoch, 4.8〜76.8 億局面) を vs Suisho 11 で測定。カーブ用測定は
   SPRT ではなく**固定 50 ペア (100 局)** で Elo ± 95% CI を点推定 (各点 ~1 時間)
2. **フェーズ b — full standard-epoch 学習**: `--superbatches 367` (1 BulletOu-epoch ≒
   1 standard-epoch = 教師全 30 ファイル 1 周) + `--max-epochs 16` で supervisor から
   長期 run を起動 (16 standard-epoch ≒ 2350 億局面 ≒ GPU ~28h)。LR step 減衰の
   周期も standard-epoch 長に伸びる (これが「2 日」解釈と整合する第一候補設定)。
   やねうら氏の実設定が判明したらそちらを優先する
3. **フェーズ c — 2^n standard-epoch checkpoint の Elo 測定**: フェーズ b の
   epoch 末 checkpoint (1, 2, 4, 8, 16 standard-epoch) をフェーズ a と同条件で測定し、
   BulletOu-epoch 系列と standard-epoch 系列の 2 本の「学習量 → Elo」カーブを得る
4. **最終判定のみ SPRT**: カーブが 0 Elo 付近に到達 (または頭打ち) した checkpoint
   に対してだけ、005 と同じ SPRT (elo0=−30, elo1=0) で「水匠級か」を判定
5. 周回学習 (>1 standard-epoch) の効果はカーブの屈曲として直接観察できる

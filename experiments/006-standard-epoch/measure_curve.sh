...
#
#   bash experiments/006-standard-epoch/measure_curve.sh a   # 005 の 2^n BulletOu-epoch 系列
#   bash experiments/006-standard-epoch/measure_curve.sh c   # 006 の 2^n standard-epoch 系列
#
# 対局条件は experiment-005 フェーズ3 と同一。GPU 学習と同時に走らせないこと
# (CPU 競合で movetime 校正が崩れる)。
set -euo pipefail

PHASE="${1:?phase を指定 (a|c)}"
...
case "$PHASE" in
  ...
  c) CKPT_BASE="$REPO/data/bulletou/checkpoints/006-std";  PREFIX="std-ep" ;;
